...
final class SecureEnclaveHelper {
    ...
    func encrypt(_ digest: Data, publicKey: SecureEnclaveKeyReference) throws -> Data {
        ...
        let result = SecKeyCreateEncryptedData(publicKey.underlying,
                                               SecKeyAlgorithm.eciesEncryptionStandardX963SHA256AESGCM, digest as CFData, &error)
        //SecKeyAlgorithm.eciesEncryptionStandardX963SHA256AESGCM, digest as CFData, &error)
    ...
    func decrypt(_ digest: Data, privateKey: SecureEnclaveKeyReference) throws -> Data {
        ...
        let result = SecKeyCreateDecryptedData(privateKey.underlying,
                                               SecKeyAlgorithm.eciesEncryptionStandardX963SHA256AESGCM, digest as CFData, &error)
        //SecKeyAlgorithm.eciesEncryptionStandardX963SHA256AESGCM, digest as CFData, &error)
